...
module.exports = me =
  globalise-entities: (json) ->
    @Nodes.set json.nodes, remove:false # add nodes first so edge comparator can read node names
    @Edges.set json.edges, remove:false
    @Evidences.set json.evidences, remove:false
    @Notes.set json.notes, remove:false
    @Maps.set json.maps, remove:false if json.maps
  ...
  init: (models) ->
    ...
    latest = c.extend do
      ...
      model: models.Latest
      parse: ->
        me.globalise-entities it.entities
        it.ids
