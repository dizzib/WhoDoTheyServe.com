...
module.exports =
  update: (req, res, next) ->
    ...
    return next new Err.Api 'Cannot update an evidenced node' if obj
  ...
  delete: (req, res, next) ->
    err, obj <- M-Edges.findOne $or:
      ...
      * b_node_id:req.id
    return next err if err
    return next new Err.Api 'Cannot delete a node with edges' if obj
    err, obj <- M-Evidences.findOne entity_id:req.id
    return next err if err
    return next new Err.Api 'Cannot delete an evidenced node' if obj
    err, maps-having-node <- get-maps-having-node req.id
    return next err if err
